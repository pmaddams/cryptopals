...
{
	BN_CTX *ctx;
	BIGNUM *r, *two, *three, *t1, *t2, *smin, *smax;
	int padding;
	...
	if (bb->i == 1) {
		...
		for (;;) {
			if (BN_mul(t1, r, bb->rsa->n, ctx) == 0 ||
			    ...
			    BN_add(smin, t1, t2) == 0 ||
			    BN_div(smin, t2, smin, bb->upper, ctx) == 0)
				goto fail;

			if (!BN_is_zero(t2))
				if (BN_add(smin, smin, BN_value_one()) == 0)
					goto fail;

			if (BN_set_word(three, 3) == 0 ||
			    BN_mul(t2, three, bb->b, ctx) == 0 ||
			    BN_add(smax, t1, t2) == 0 ||
			    BN_div(smax, t2, smax, bb->lower, ctx) == 0)
				goto fail;

			if (!BN_is_zero(t2))
				if (BN_add(smax, smax, BN_value_one()) == 0)
					goto fail;

			if (BN_copy(bb->s, smin) == 0)
				goto fail;
